...
export function decodeBuffer(buffer: Buffer, charset?: string | null): string {
...
export async function guessStreamInfo(
	...
	if (!baseGuess.mimetype && baseGuess.extension) {
		...
		if (guessMime) {
			enhancedGuess = enhancedGuess.copyAndUpdate({
				mimetype: String(guessMime),
			})
		}
	}
		...
		if (compatible) {
			guesses.push(
				baseGuess.copyAndUpdate({
					...
					charset:
						baseGuess.charset ?? maybeGuessCharset(buffer, fileType.mime),
				}),
			)
		} else {
	...
	guesses.push(
		enhancedGuess.copyAndUpdate({
			charset:
				enhancedGuess.charset ??
				maybeGuessCharset(buffer, enhancedGuess.mimetype),
		}),
	)
	return guesses
}
