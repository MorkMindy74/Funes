...
import { ensureLeadingDot, normalizeCharset, readBinarySource } from "./utils/stream.js";
...
async function main(): Promise<void> {
  ...
  if (values["list-plugins"]) {
    process.stdout.write("Installed MarkItDown 3rd-party Plugins:\n\n  * No native TypeScript plugin discovery is configured yet.\n");
    return;
  }

  if (values["use-docintel"] && !values.endpoint) {
    process.stderr.write("Document Intelligence Endpoint is required when using Document Intelligence.\n");
    process.exitCode = 1;
    return;
  }
