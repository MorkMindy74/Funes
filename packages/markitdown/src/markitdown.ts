...
export class MarkItDown {
	...
	async convertLocal(
		...
		if (options.preferPython) {
			return this.pythonBridge.convertPath(
				targetPath,
				baseGuess,
		...
		return this.convertBufferWithFallback(buffer, baseGuess, options, () =>
			this.pythonBridge.convertPath(
				targetPath,
				baseGuess,
				this.mergeOptions(options),
			),
	...
	async convertBuffer(
		...
		if (options.preferPython) {
			return this.pythonBridge.convertBuffer(
				buffer,
				baseGuess,
				this.mergeOptions(options),
			)
		...
		return this.convertBufferWithFallback(buffer, baseGuess, options, () =>
			this.pythonBridge.convertBuffer(
				buffer,
				baseGuess,
				this.mergeOptions(options),
			),
	...
	async convertUri(
		...
		if (/^https?:/i.test(trimmed)) {
			if (options.preferPython) {
				return this.pythonBridge.convertUri(
					trimmed,
			...
			return this.convertResponse(
				response,
				new StreamInfo({ url: trimmed }).copyAndUpdate(streamInfo),
				options,
			)
		}
	...
	async convertResponse(
		...
		const [mimetypePart, ...params] = (contentType ?? "")
			.split(";")
			.map((part) => part.trim())
	...
	private async convertNative(
		...
		const registrations = [...this.converters].sort(
		...
		for (const streamInfo of [...streamInfoGuesses, new StreamInfo()]) {
			for (const registration of registrations) {
				...
				const accepts = await registration.converter.accepts(
					input,
					mergedOptions,
				)
				if (!accepts) {
				...
				try {
					const result = await registration.converter.convert(
						input,
						mergedOptions,
					)
					...
					failures.push(
						new FailedConversionAttempt(
							registration.converter.constructor.name,
							error,
						),
					)
				}
