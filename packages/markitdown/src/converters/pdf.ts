		...
		try {
			...
			for (
				...
				try {
					...
					const formContent = extractFormContentFromItems(
						content.items,
						viewport.width,
					)
					const pageContent =
						formContent ??
						extractPlainTextFromItems(content.items, viewport.width)
...
function extractFormContentFromItems(
	items: unknown[],
	pageWidth: number,
): string | null {
	const rows = buildRows(items)
	...
	const rowInfo: TableRowInfo[] = rows.map((row) => {
		...
		return {
			...
			hasPartialNumbering:
				Boolean(firstWord) && PARTIAL_NUMBERING_PATTERN.test(firstWord.text),
		}
	})
	...
	const columnsPerInch =
		globalColumns.length / Math.max(contentWidth / 72, 0.01)
	...
	for (let index = 0; index < rowInfo.length; ) {
		...
		while (
			...
			(rowInfo[expandedStart - 1]?.y ?? 0) - (rowInfo[expandedStart]?.y ?? 0) <=
				30
		) {
			expandedStart -= 1
		}
...
function extractPositionedWords(items: unknown[]): PositionedWord[] {
	...
	for (const item of items) {
		...
		const height = Math.abs(
			item.height ?? item.transform[0] ?? item.transform[3] ?? 0,
		)
...
function appendTextRow(
	...
	if (previousRow) {
		...
		const isParagraphBreak =
			...
			(verticalGap > baselineGap * 1.2 &&
				row.words[0]!.x0 - 10 < pageWidth * 0.2)
...
function formatMarkdownTable(rows: string[][]): string {
	...
	const columnWidths = nonEmptyRows[0]!.map((_, columnIndex) =>
		Math.max(3, ...nonEmptyRows.map((row) => (row[columnIndex] ?? "").length)),
	)
...
function buildLineText(words: PositionedWord[]): string {
	...
	for (const word of words) {
		...
		const shouldInsertSpace =
			!line.endsWith("-") && !/^[,.;:!?%)]/.test(word.text) && gap >= -0.5
