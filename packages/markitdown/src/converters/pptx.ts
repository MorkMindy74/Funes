...
function getShapeEntries(tree: Record<string, unknown>): ShapeEntry[] {
  ...
  for (const [key, value] of Object.entries(tree)) {
    ...
    for (const node of toArray(value as Record<string, unknown> | Record<string, unknown>[])) {
...
function getShapePosition(node: Record<string, unknown>): { x: number; y: number } {
  const xfrm =
...
async function processPicture(
  ...
  const streamInfo = new StreamInfo({
    extension,
    mimetype: String(mime.lookup(`placeholder${extension}`) || "application/octet-stream"),
  ...
  const altText = sanitizeAltText([llmDescription, descr].filter(Boolean).join("\n") || shapeName);
...
function tableToMarkdown(table: Record<string, unknown>): string {
  const rows = findChildren<Record<string, unknown>>(table, "tr").map((row) =>
    findChildren<Record<string, unknown>>(row, "tc").map((cell) =>
      toArray(findChild(cell, "txBody"))
        .flatMap((txBody) => toArray(findChild(txBody as Record<string, unknown>, "p")))
        .map((paragraph) => extractText(paragraph).trim())
        .join(" ")
        .trim(),
    ),
  ...
  const normalized = rows.map((row) =>
    [...row, ...Array(Math.max(0, width - row.length)).fill("")].slice(0, width),
  );
...
function chartToMarkdown(chartXml: string): string {
  try {
    ...
    const rows = categories.map((category, index) => [
      category,
      ...series.map((item) => readChartPoints(findChild(item, "val"))[index] ?? ""),
    ]);
...
function readChartPoints(node: unknown): string[] {
  ...
  return findChildren<Record<string, unknown>>(cache, "pt")
    .sort(
      (left, right) =>
        Number(getAttribute(left, "idx") ?? 0) - Number(getAttribute(right, "idx") ?? 0),
    )
...
async function parseRelationships(
  ...
  const relationships = findChild<Record<string, unknown>>(parsed, "Relationships");
  for (const relationship of findChildren<Record<string, unknown>>(relationships, "Relationship")) {
