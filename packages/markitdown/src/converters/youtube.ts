...
export class YouTubeConverter extends DocumentConverter {
	accepts({
		...
		return (
			...
			(ACCEPTED_FILE_EXTENSIONS.includes(extension) ||
				ACCEPTED_MIME_TYPE_PREFIXES.some((prefix) =>
	...
	async convert(
		...
		const description = findNestedValue(
			initialData,
		...
		const textDescription = firstValue(metadata, [
			"description",
			"og:description",
		])
		if (textDescription) {
		...
		const transcript = await fetchTranscript(streamInfo.url, options).catch(
			() => undefined,
		)
		if (transcript) {
			markdown += `\n### Transcript\n${transcript}\n`
