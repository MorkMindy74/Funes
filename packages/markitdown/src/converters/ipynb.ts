...
export class IpynbConverter extends DocumentConverter {
  accepts({ buffer, streamInfo }: Parameters<DocumentConverter["accepts"]>[0]): boolean {
    ...
    if (CANDIDATE_MIME_TYPE_PREFIXES.some((prefix) => mimetype.startsWith(prefix))) {
      const content = decodeBuffer(buffer, streamInfo.charset);
      return content.includes('"nbformat"') && content.includes('"nbformat_minor"');
    }
  ...
  convert(
    ...
    try {
      ...
      throw new FileConversionError(
        error instanceof Error ? error.message : "Error converting .ipynb file.",
      );
    }
  }
}
