...
const ACCEPTED_MIME_TYPE_PREFIXES = ["application/epub", "application/epub+zip", "application/x-epub+zip"];
...
export class EpubConverter extends DocumentConverter {
  ...
  async convert(
    ...
    const metadataLines = [
      ...
      formatMetadata("Authors", arrayify(metadata?.["dc:creator"]).map(textNode).filter(Boolean).join(", ")),
