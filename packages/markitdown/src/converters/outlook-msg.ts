...
export class OutlookMsgConverter extends DocumentConverter {
  ...
  convert(
    ...
    const lines = [
      ...
      toRecipients.length > 0 ? `**To:** ${toRecipients.join(", ")}` : undefined,
...
function resolveMsgReader(): new (buffer: Buffer) => { getFileData(): unknown } {
  const candidate =
    ((msgreader as unknown as { default?: { default?: unknown } }).default?.default as
      | (new (buffer: Buffer) => { getFileData(): unknown })
      | undefined) ??
