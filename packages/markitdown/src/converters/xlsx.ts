...
const ACCEPTED_XLSX_MIME_TYPE_PREFIXES = [
	"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
]
const ACCEPTED_XLSX_FILE_EXTENSIONS = [".xlsx"]

const ACCEPTED_XLS_MIME_TYPE_PREFIXES = [
...
export class XlsxConverter extends DocumentConverter {
	accepts({
		...
		return (
			...
			ACCEPTED_XLSX_MIME_TYPE_PREFIXES.some((prefix) =>
...
export class XlsConverter extends DocumentConverter {
	accepts({
		...
		return (
			...
			ACCEPTED_XLS_MIME_TYPE_PREFIXES.some((prefix) =>
				mimetype.startsWith(prefix),
			)
		)
	}
