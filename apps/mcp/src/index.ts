...
app.get("/.well-known/oauth-authorization-server", async (c) => {
  try {
    ...
    logger.error("Error fetching OAuth metadata", { error: error instanceof Error ? error.message : error })
...
app.all("/mcp", async (c) => {
  ...
  let authUser: { userId: string; apiKey: string; email?: string; name?: string } | null = null
