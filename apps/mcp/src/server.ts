...
export function createMcpServer(props: UserProps): McpServer {
	...
	server.tool(
		...
		async (args) => {
			...
			const effectiveContainerTag =
				(args as any).containerTag || props.containerTag
				...
				if (!response.ok) {
					...
					return {
						content: [
							{
								type: "text" as const,
								text: `Error converting document: ${(errorData as any).error || response.statusText}`,
							},
						],
						isError: true,
					}
	...
	// ─── Resources ───────────────────────────────────────────────────
	server.resource("User Profile", "supermemory://profile", async () => {
		const client = getClient(props)
	...
	server.resource("My Projects", "supermemory://projects", async () => {
		const client = getClient(props)
		const projects = await client.getProjects()
		return {
			contents: [
	...
	server.prompt(
		...
		async (args) => {
			try {
				...
				const parts: string[] = []
				parts.push(
					"**Important:** Whenever the user shares informative facts, preferences, personal details, or any memory-worthy information, use the `memory` tool to save it. This helps maintain context across conversations.",
				)
				parts.push("")
