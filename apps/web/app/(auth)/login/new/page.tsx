"use client"

import { signIn, authClient } from "@lib/auth"
import { usePostHog } from "@lib/posthog"
import { TextSeparator } from "@ui/components/text-separator"
import { ExternalAuthButton } from "@ui/button/external-auth"
import { Button } from "@ui/components/button"
import { Badge } from "@ui/components/badge"
import { LabeledInput } from "@ui/input/labeled-input"
import { HeadingH3Medium } from "@ui/text/heading/heading-h3-medium"
import { Label1Regular } from "@ui/text/label/label-1-regular"
import { Title1Bold } from "@ui/text/title/title-1-bold"
import { InitialHeader } from "@/components/initial-header"
import { useRouter, useSearchParams } from "next/navigation"
import { useState, useEffect } from "react"
import { motion } from "motion/react"
import { dmSansClassName } from "@/lib/fonts"
import { cn } from "@lib/utils"
import { Logo } from "@ui/assets/Logo"

function AnimatedGradientBackground() {
	return (
		<div className="fixed inset-0 z-0 overflow-hidden">
			<motion.div
				className="absolute top-[20%] left-0 right-0 bottom-0 bg-[url('/onboarding/bg-gradient-0.png')] bg-size-[150%_auto] bg-top bg-no-repeat"
				initial={{ y: "100%" }}
				animate={{
					y: 0,
					opacity: [1, 0, 1],
				}}
				transition={{
					y: { duration: 0.75, ease: "easeOut" },
					opacity: {
						duration: 8,
						repeat: Number.POSITIVE_INFINITY,
						ease: "easeInOut",
					},
				}}
			/>
			<motion.div
				className="absolute top-[20%] left-0 right-0 bottom-0 bg-[url('/onboarding/bg-gradient-1.png')] bg-size-[150%_auto] bg-top bg-no-repeat"
				initial={{ y: "100%" }}
				animate={{
					y: 0,
					opacity: [0, 1, 0],
				}}
				transition={{
					y: { duration: 0.75, ease: "easeOut" },
					opacity: {
						duration: 8,
						repeat: Number.POSITIVE_INFINITY,
						ease: "easeInOut",
					},
				}}
			/>
			<motion.div
				className="absolute top-0 left-0 right-0 bottom-0 bg-[url('/bg-rectangle.png')] bg-cover bg-center bg-no-repeat"
				transition={{ duration: 0.75, ease: "easeOut", bounce: 0 }}
				style={{
					mixBlendMode: "soft-light",
					opacity: 0.4,
				}}
			/>
		</div>
	)
}

function LoginCard({ children }: { children: React.ReactNode }) {
	return (
		<motion.div
			className="flex py-8 px-11 flex-col items-start gap-2 rounded-[22px] bg-linear-to-b from-[#06101F] to-[#030912] shadow-[1.5px_1.5px_20px_0_rgba(0,0,0,0.65),1px_1.5px_2px_0_rgba(128,189,255,0.07)_inset,-0.5px_-1.5px_4px_0_rgba(0,35,73,0.40)_inset]"
			initial={{ opacity: 0, y: 20 }}
			animate={{ opacity: 1, y: 0 }}
			transition={{ duration: 0.75, ease: "easeOut" }}
		>
			{children}
		</motion.div>
	)
}

export default function LoginPage() {
	const [email, setEmail] = useState("")
	const [password, setPassword] = useState("")
	const [isLoading, setIsLoading] = useState(false)
	const [error, setError] = useState<string | null>(null)
	const [lastUsedMethod, setLastUsedMethod] = useState<string | null>(null)
	const [mode, setMode] = useState<"password" | "magic_link">("password")
	const [submittedEmail, setSubmittedEmail] = useState<string | null>(null)
	const router = useRouter()

	const posthog = usePostHog()
	const params = useSearchParams()

	const redirectUrl = params.get("redirect")

	const getCallbackURL = () => {
		const origin = window.location.origin
		let finalUrl: URL
		if (redirectUrl) {
			try {
				finalUrl = new URL(redirectUrl, origin)
			} catch {
				finalUrl = new URL(origin)
			}
		} else {
			finalUrl = new URL(origin)
		}
		finalUrl.searchParams.set("extension-auth-success", "true")
		return finalUrl.toString()
	}

	useEffect(() => {
		const savedMethod = localStorage.getItem("funes-last-login-method")
		setLastUsedMethod(savedMethod)
	}, [])

	function saveLoginMethod(method: string) {
		try {
			localStorage.setItem("funes-last-login-method", method)
		} catch {}
	}

	useEffect(() => {
		if (params.get("error")) {
			setError(params.get("error"))
		}
	}, [params])

	// ─── Email + Password Login ────────────────────────────────────

	const handlePasswordLogin = async (e: React.FormEvent<HTMLFormElement>) => {
		e.preventDefault()
		setIsLoading(true)
		setError(null)

		posthog.capture("login_attempt", { method: "password" })

		try {
			const result = await authClient.signIn.email({
				email,
				password,
				callbackURL: getCallbackURL(),
			})

			if (result.error) {
				setError(result.error.message || "Invalid email or password")
				setIsLoading(false)
				return
			}

			saveLoginMethod("password")
			// Redirect — better-auth sets session cookie
			router.push(getCallbackURL())
		} catch (err) {
			console.error(err)
			if (err instanceof Error && err.message.toLowerCase().includes("fetch")) {
				setError("Cannot connect to backend. Is the server running?")
			} else {
				setError(
					err instanceof Error ? err.message : "Login failed. Please try again.",
				)
			}
			setIsLoading(false)
		}
	}

	// ─── Magic Link Login ──────────────────────────────────────────

	const handleMagicLink = async (e: React.FormEvent<HTMLFormElement>) => {
		e.preventDefault()
		setIsLoading(true)
		setError(null)

		try {
			await signIn.magicLink({ callbackURL: getCallbackURL(), email })
			setSubmittedEmail(email)
			saveLoginMethod("magic_link")
		} catch (err) {
			console.error(err)
			setError(
				err instanceof Error ? err.message : "Failed to send magic link",
			)
		} finally {
			setIsLoading(false)
		}
	}

	const handleSubmitToken = async (event: React.FormEvent<HTMLFormElement>) => {
		event.preventDefault()
		setIsLoading(true)
		const formData = new FormData(event.currentTarget)
		const token = formData.get("token") as string
		const callbackURL = getCallbackURL()
		router.push(
			`${process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:3001"}/api/auth/magic-link/verify?token=${token}&callbackURL=${encodeURIComponent(callbackURL)}`,
		)
	}

	// ─── Social Auth Handlers ──────────────────────────────────────

	const handleSocial = (provider: "google" | "github") => {
		if (isLoading) return
		setIsLoading(true)
		posthog.capture("login_attempt", { method: "social", provider })
		saveLoginMethod(provider)
		signIn
			.social({ callbackURL: getCallbackURL(), provider })
			.finally(() => setIsLoading(false))
	}

	return (
		<main className="relative h-screen overflow-hidden">
			<AnimatedGradientBackground />
			<div className="relative z-10">
				<InitialHeader />
				<section className="flex flex-col items-center justify-center p-4 space-y-12 sm:p-6 md:p-8 lg:px-20 lg:py-12.5 min-h-[calc(100vh-80px)]">
					<div className="text-center">
						<div className="text-5xl font-medium">
							Never forget anything, anywhere
						</div>
						<div className="text-5xl font-medium">with Funes</div>
					</div>

					{/* Magic Link Verification View */}
					{submittedEmail ? (
						<LoginCard>
							<div className="w-[360px] flex flex-col gap-4 lg:gap-6 min-h-2/3">
								<div className="flex flex-col gap-2 text-center lg:text-left">
									<Title1Bold className="text-foreground">
										Almost there!
									</Title1Bold>
									<HeadingH3Medium className="text-muted-foreground">
										Click the magic link we&apos;ve sent to{" "}
										<span className="text-foreground">{submittedEmail}</span>.
									</HeadingH3Medium>
								</div>

								<TextSeparator text="OR" className={cn(dmSansClassName())} />

								<form
									className="flex flex-col gap-4 lg:gap-6"
									onSubmit={handleSubmitToken}
								>
									<LabeledInput
										inputPlaceholder="your temporary login code"
										inputProps={{
											name: "token",
											required: true,
											disabled: isLoading,
										}}
										inputType="text"
										label="Enter code"
									/>
									<Button disabled={isLoading} type="submit">
										Verify Token
									</Button>
								</form>

								<button
									type="button"
									onClick={() => setSubmittedEmail(null)}
									className="text-sm text-muted-foreground hover:text-foreground text-center"
								>
									Back to login
								</button>
							</div>
						</LoginCard>
					) : (
						<LoginCard>
							<div className="w-[360px] flex flex-col" style={{ gap: "12px" }}>
								{error && (
									<div className="text-red-500 text-sm bg-red-900/20 border border-red-800/50 rounded-lg px-3 py-2">
										{error}
									</div>
								)}

								{/* Social Auth Buttons */}
								<div className="flex flex-col gap-3">
									{(process.env.NEXT_PUBLIC_HOST_ID === "supermemory" ||
										!process.env.NEXT_PUBLIC_GOOGLE_AUTH_ENABLED) && (
										<div className="relative grow">
											<ExternalAuthButton
												authIcon={
													<svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" height="25" viewBox="0 0 24 25" width="24" xmlns="http://www.w3.org/2000/svg">
														<title>Google</title>
														<path d="M21.8055 10.2563H21V10.2148H12V14.2148H17.6515C16.827 16.5433 14.6115 18.2148 12 18.2148C8.6865 18.2148 6 15.5283 6 12.2148C6 8.90134 8.6865 6.21484 12 6.21484C13.5295 6.21484 14.921 6.79184 15.9805 7.73434L18.809 4.90584C17.023 3.24134 14.634 2.21484 12 2.21484C6.4775 2.21484 2 6.69234 2 12.2148C2 17.7373 6.4775 22.2148 12 22.2148C17.5225 22.2148 22 17.7373 22 12.2148C22 11.5443 21.931 10.8898 21.8055 10.2563Z" fill="#FFC107" />
														<path d="M3.15234 7.56034L6.43784 9.96984C7.32684 7.76884 9.47984 6.21484 11.9993 6.21484C13.5288 6.21484 14.9203 6.79184 15.9798 7.73434L18.8083 4.90584C17.0223 3.24134 14.6333 2.21484 11.9993 2.21484C8.15834 2.21484 4.82734 4.38334 3.15234 7.56034Z" fill="#FF3D00" />
														<path d="M12.0002 22.2152C14.5832 22.2152 16.9302 21.2267 18.7047 19.6192L15.6097 17.0002C14.5721 17.7897 13.3039 18.2166 12.0002 18.2152C9.39916 18.2152 7.19066 16.5567 6.35866 14.2422L3.09766 16.7547C4.75266 19.9932 8.11366 22.2152 12.0002 22.2152Z" fill="#4CAF50" />
														<path d="M21.8055 10.2563H21V10.2148H12V14.2148H17.6515C17.2571 15.3231 16.5467 16.2914 15.608 17.0003L15.6095 16.9993L18.7045 19.6183C18.4855 19.8173 22 17.2148 22 12.2148C22 11.5443 21.931 10.8898 21.8055 10.2563Z" fill="#1976D2" />
													</svg>
												}
												authProvider="Google"
												className="w-full"
												disabled={isLoading}
												onClick={() => handleSocial("google")}
											/>
											{lastUsedMethod === "google" && (
												<div className="absolute -top-2 -right-2">
													<Badge variant="default" className="text-xs">Last used</Badge>
												</div>
											)}
										</div>
									)}
									{(process.env.NEXT_PUBLIC_HOST_ID === "supermemory" ||
										!process.env.NEXT_PUBLIC_GITHUB_AUTH_ENABLED) && (
										<div className="relative grow">
											<ExternalAuthButton
												authIcon={
													<svg className="w-4 h-4 sm:w-5 sm:h-5 text-foreground" fill="none" height="25" viewBox="0 0 26 25" width="26" xmlns="http://www.w3.org/2000/svg">
														<title>Github</title>
														<g clipPath="url(#clip0_2579_3356)">
															<path clipRule="evenodd" d="M12.9635 0.214844C6.20975 0.214844 0.75 5.71484 0.75 12.5191C0.75 17.9581 4.24825 22.5621 9.10125 24.1916C9.708 24.3141 9.93025 23.9268 9.93025 23.6011C9.93025 23.3158 9.91025 22.3381 9.91025 21.3193C6.51275 22.0528 5.80525 19.8526 5.80525 19.8526C5.25925 18.4266 4.45025 18.0601 4.45025 18.0601C3.33825 17.3063 4.53125 17.3063 4.53125 17.3063C5.76475 17.3878 6.412 18.5693 6.412 18.5693C7.50375 20.4433 9.263 19.9138 9.97075 19.5878C10.0718 18.7933 10.3955 18.2433 10.7393 17.9378C8.0295 17.6526 5.1785 16.5933 5.1785 11.8671C5.1785 10.5226 5.6635 9.42259 6.432 8.56709C6.31075 8.26159 5.886 6.99834 6.5535 5.30759C6.5535 5.30759 7.58475 4.98159 9.91 6.57059C10.9055 6.30126 11.9322 6.16425 12.9635 6.16309C13.9948 6.16309 15.046 6.30584 16.0168 6.57059C18.3423 4.98159 19.3735 5.30759 19.3735 5.30759C20.041 6.99834 19.616 8.26159 19.4948 8.56709C20.2835 9.42259 20.7485 10.5226 20.7485 11.8671C20.7485 16.5933 17.8975 17.6321 15.1675 17.9378C15.6125 18.3248 15.9965 19.0581 15.9965 20.2193C15.9965 21.8693 15.9765 23.1936 15.9765 23.6008C15.9765 23.9268 16.199 24.3141 16.8055 24.1918C21.6585 22.5618 25.1568 17.9581 25.1568 12.5191C25.1768 5.71484 19.697 0.214844 12.9635 0.214844Z" fill="currentColor" fillRule="evenodd" />
														</g>
														<defs>
															<clipPath id="clip0_2579_3356">
																<rect fill="currentColor" height="24" transform="translate(0.75 0.214844)" width="24.5" />
															</clipPath>
														</defs>
													</svg>
												}
												authProvider="Github"
												className="w-full"
												disabled={isLoading}
												onClick={() => handleSocial("github")}
											/>
											{lastUsedMethod === "github" && (
												<div className="absolute -top-2 -right-2">
													<Badge variant="default" className="text-xs">Last used</Badge>
												</div>
											)}
										</div>
									)}
								</div>

								<TextSeparator text="OR" className={cn(dmSansClassName())} />

								{/* Email + Password Login (default for self-hosted) */}
								{mode === "password" ? (
									<form onSubmit={handlePasswordLogin} className="flex flex-col gap-4">
										<LabeledInput
											error={null}
											inputPlaceholder="your@email.com"
											inputProps={{
												disabled: isLoading,
												id: "email",
												onChange: (e: React.ChangeEvent<HTMLInputElement>) => {
													setEmail(e.target.value)
													error && setError(null)
												},
												required: true,
												value: email,
											}}
											inputType="email"
										/>

										<LabeledInput
											error={null}
											inputPlaceholder="your password"
											inputProps={{
												disabled: isLoading,
												id: "password",
												onChange: (e: React.ChangeEvent<HTMLInputElement>) => {
													setPassword(e.target.value)
													error && setError(null)
												},
												required: true,
												value: password,
											}}
											inputType="password"
										/>

										<div className="relative">
											<Button
												className="flex justify-center items-center w-full h-[44px] relative gap-3 p-2 rounded-xl"
												style={{
													background:
														"linear-gradient(182.37deg, #0ff0d2 -91.53%, #5bd3fb -67.8%, #1e0ff0 95.17%)",
													boxShadow:
														"1px 1px 2px 0px #1A88FF inset, 0 2px 10px 0 rgba(5, 1, 0, 0.20)",
												}}
												disabled={isLoading}
												type="submit"
											>
												<Logo className="size-4" />
												{isLoading ? "Signing in..." : "Sign in"}
											</Button>
											{lastUsedMethod === "password" && (
												<div className="absolute -top-2 -right-2">
													<Badge variant="default" className="text-xs">Last used</Badge>
												</div>
											)}
										</div>

										<button
											type="button"
											onClick={() => setMode("magic_link")}
											className="text-xs text-muted-foreground hover:text-foreground text-center"
										>
											Use magic link instead
										</button>
									</form>
								) : (
									/* Magic Link Login */
									<form onSubmit={handleMagicLink} className="flex flex-col gap-4">
										<LabeledInput
											error={null}
											inputPlaceholder="your@email.com"
											inputProps={{
												disabled: isLoading,
												id: "email-magic",
												onChange: (e: React.ChangeEvent<HTMLInputElement>) => {
													setEmail(e.target.value)
													error && setError(null)
												},
												required: true,
												value: email,
											}}
											inputType="email"
										/>

										<div className="relative">
											<Button
												className="flex justify-center items-center w-full h-[44px] relative gap-3 p-2 rounded-xl"
												style={{
													background:
														"linear-gradient(182.37deg, #0ff0d2 -91.53%, #5bd3fb -67.8%, #1e0ff0 95.17%)",
													boxShadow:
														"1px 1px 2px 0px #1A88FF inset, 0 2px 10px 0 rgba(5, 1, 0, 0.20)",
												}}
												disabled={isLoading}
												type="submit"
											>
												<Logo className="size-4" />
												{isLoading ? "Sending link..." : "Send magic link"}
											</Button>
											{lastUsedMethod === "magic_link" && (
												<div className="absolute -top-2 -right-2">
													<Badge variant="default" className="text-xs">Last used</Badge>
												</div>
											)}
										</div>

										<button
											type="button"
											onClick={() => setMode("password")}
											className="text-xs text-muted-foreground hover:text-foreground text-center"
										>
											Use password instead
										</button>
									</form>
								)}

								<Label1Regular
									className={cn(
										"text-center text-xs! text-[#737373B2]",
										dmSansClassName(),
									)}
								>
									First time?{" "}
									<a className="underline" href="/setup">
										Set up Funes
									</a>
								</Label1Regular>
							</div>
						</LoginCard>
					)}
				</section>
			</div>
		</main>
	)
}
