...
async function testBasicWorkflow(npmPrefix) {
	...
	try {
		...
		const createOutput = exec(
			`"${bdCmd}" create "Test issue" -t task -p 1 --json`,
			{
				cwd: projectDir,
				env,
			},
		...
		const showOutput = exec(`"${bdCmd}" show ${issue.id} --json`, {
			cwd: projectDir,
			env,
		})
		...
		if (!showIssue.id || showIssue.id !== issue.id) {
			throw new Error(
				`bd show returned wrong issue: expected ${issue.id}, got ${showIssue.id}`,
			)
		}
		...
		const updatedOutput = exec(`"${bdCmd}" show ${issue.id} --json`, {
			cwd: projectDir,
			env,
		})
		const updatedResult = JSON.parse(updatedOutput)
		const updatedIssue = Array.isArray(updatedResult)
			? updatedResult[0]
			: updatedResult
		...
		exec(`"${bdCmd}" close ${issue.id} --reason "Test completed"`, {
			cwd: projectDir,
			env,
		})
		const closedOutput = exec(`"${bdCmd}" show ${issue.id} --json`, {
			cwd: projectDir,
			env,
		})
		const closedResult = JSON.parse(closedOutput)
		const closedIssue = Array.isArray(closedResult)
			? closedResult[0]
			: closedResult
		...
		const readyOutput = exec(`"${bdCmd}" ready --json`, {
			cwd: projectDir,
			env,
		})
...
async function testClaudeCodeWebSimulation(npmPrefix) {
	...
	try {
		...
		if (!issues.some((i) => i.id === existingIssue.id)) {
			throw new Error(
				`Existing issue ${existingIssue.id} not imported from JSONL`,
			)
		}
