...
async function waitForFileAccess(filePath, timeoutMs = 30000) {
	...
	console.warn(
		`Warning: file ${path.basename(filePath)} may still be locked after ${timeoutMs}ms, attempting extraction anyway...`,
	)
}
	...
	for (let attempt = 1; attempt <= maxRetries; attempt++) {
		try {
			...
			if (isFileLockError && attempt < maxRetries) {
				...
				console.log(
					`File appears locked (attempt ${attempt}/${maxRetries}). Retrying in ${delayMs}ms...`,
				)
				await sleep(delayMs)
			} else if (attempt === maxRetries) {
				throw new Error(
					`Failed to extract archive after ${maxRetries} attempts: ${err.message}\n${stderr}`,
				)
			} else {
				throw new Error(`Failed to extract archive: ${err.message}\n${stderr}`)
			}
...
async function install() {
	try {
		...
		console.error(
			"1. Installing manually from: https://github.com/steveyegge/beads/releases",
		)
		console.error(
			"2. Using the install script: curl -fsSL https://raw.githubusercontent.com/steveyegge/beads/main/scripts/install.sh | bash",
		)
		console.error(
			"3. Opening an issue: https://github.com/steveyegge/beads/issues",
		)
		process.exit(1)
	}
}
